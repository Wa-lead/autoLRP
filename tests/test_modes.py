...
Flipping a config (``LRPConfig.composite()``, ``'yx'`` on the activation
nodes, ``'gate'``/``'jacobian'`` on the softmax nodes) must produce an observably different
...
from autolrp import BASE, on, ELEMENTWISE_NODES, SOFTMAX_NODES
...
def test_activation_yx_differs_from_passthrough(act_cls):
    """``'yx'`` on the activation nodes produces a different heatmap than ``'passthrough'``."""
    ...
    def run(mode):
        x = autolrp.tensor(data.clone())
        act(lin(x)).sum().lrp(config=LRPConfig(rule={**BASE, **on(ELEMENTWISE_NODES, mode)}))
        return x.relevance.clone()
...
def test_relu_yx_zeros_dead_preactivations():
    """Under ``'yx'`` on the activation nodes, positions where ReLU's preactivation ≤ 0
    ...
    x = autolrp.tensor(data.clone())
    F.relu(lin(x)).sum().lrp(config=LRPConfig(rule={**BASE, **on(ELEMENTWISE_NODES, 'yx')}))
...
def test_softmax_modes_distinct(mode_a, mode_b):
    ...
    def run(mode):
        ...
        out[0, 0].lrp(config=LRPConfig(rule={**BASE, **on(SOFTMAX_NODES, mode)}))
        return x.relevance.clone()
...
def test_softmax_jacobian_zeros_dead_positions():
    ...
    x = autolrp.tensor(data.clone())
    torch.softmax(x, dim=-1).sum().lrp(config=LRPConfig(rule={**BASE, **on(SOFTMAX_NODES, 'jacobian')}))
